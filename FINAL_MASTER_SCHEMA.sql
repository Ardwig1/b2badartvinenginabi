-- ========================================================
-- B2B PLATFORM - FINAL MASTER SCHEMA (V4)
-- ========================================================
-- Hazırlayan: Gemini CLI (Yağız adına)
-- Bu SQL dosyası sıfır bir Supabase projesini Engin Abi 
-- veya herhangi bir yeni müşteri için TAM hazır hale getirir.
-- 19 Tablo, RLS Kuralları, Fonksiyonlar ve Triggerlar dahildir.
...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  role text DEFAULT 'customer', -- 'customer', 'admin', 'representative'
  created_at timestamptz DEFAULT now()
);
...
-- 15. ACCOUNT TRANSACTIONS (Cari Hareketler)
CREATE TABLE IF NOT EXISTS public.account_transactions (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    company_id uuid REFERENCES public.companies(id) ON DELETE CASCADE,
    type text NOT NULL CHECK (type IN ('payment', 'invoice', 'refund', 'adjustment')),
...
CREATE TABLE IF NOT EXISTS public.company_extra_discounts (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    company_id uuid REFERENCES public.companies(id) ON DELETE CASCADE,
    category text NOT NULL, -- 'General', 'Filter', 'Oil' etc.
...
-- Simple Admin Check Policy
CREATE POLICY "Admins manage everything" ON public.products FOR ALL USING (
  EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_admin = true)
);

CREATE POLICY "Users read products" ON public.products FOR SELECT USING (auth.uid() IS NOT NULL);
