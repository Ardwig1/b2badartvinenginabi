-- ========================================================
-- B2B PLATFORM - FINAL 1:1 MASTER SCHEMA (V6 - FULL SCAN)
-- ========================================================
-- Hazırlayan: Gemini CLI (Yağız adına)
-- Kaan Abi'nin projesindeki tüm tabloları ve kolonları 
-- BİREBİR (1:1) karşılar. Kodu değiştirmeye gerek kalmaz.
...
-- 2. COMPANIES (All fields from Admin & Checkout logic)
CREATE TABLE IF NOT EXISTS public.companies (
    ...
    risk_limit numeric DEFAULT 0, -- Critical for order logic
    current_balance numeric DEFAULT 0,
    is_prepayment_locked boolean DEFAULT false, -- Critical for prepayment logic
    risk_limit_unlimited boolean DEFAULT false,
    extra_discount_percent numeric DEFAULT 0,
    dealer_code text, -- Used in login
    user_code text,   -- Used in login
    password text,    -- Plain text sync
    created_at timestamptz DEFAULT now()
);
...
-- 4. PRODUCTS (Including stock bins and campaign status)
CREATE TABLE IF NOT EXISTS public.products (
    ...
    stock_merkez integer DEFAULT 0, -- Added for bin logic
    stock_depo integer DEFAULT 0,   -- Added for bin logic
    unit text DEFAULT 'adet',
    is_active boolean DEFAULT true,
    is_campaign boolean DEFAULT false, -- Added for campaign logic
    created_at timestamptz DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS public.orders (
    ...
    is_stock_reduced boolean DEFAULT false, -- Critical for Admin sync
    document_no text UNIQUE, -- OMG number
    created_at timestamptz DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS public.order_items (
    ...
    shipping_company text, -- Added from fix_item_shipping
    tracking_number text,  -- Added from fix_item_shipping
    shipping_origin text   -- Added from fix_item_shipping
);
...
CREATE TABLE IF NOT EXISTS public.account_transactions (
    ...
    order_id uuid REFERENCES public.orders(id) ON DELETE SET NULL, -- Specified by user
...
CREATE TABLE IF NOT EXISTS public.cart_items (
    ...
    created_at timestamptz DEFAULT now(), -- Specified by user
    updated_at timestamptz DEFAULT now(),
    UNIQUE(company_id, product_id)
);
...
CREATE TRIGGER set_document_no_trigger BEFORE INSERT ON public.account_transactions FOR EACH ROW EXECUTE FUNCTION public.trg_set_document_no();
...
CREATE POLICY "Authenticated users read products" ON public.products FOR SELECT USING (auth.uid() IS NOT NULL);
...
-- INITIAL DATA
-- ========================================================
INSERT INTO public.price_groups (name, discount_percent) VALUES ('A Grubu', 20), ('B Grubu', 15), ('C Grubu', 10) ON CONFLICT DO NOTHING;
