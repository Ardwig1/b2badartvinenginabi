...
export async function createPaymentLink({ token, merchantKey, appSecret, invoiceId, amount, returnUrl, cancelUrl, name, surname, items }) {
    // Hash verisi: total|installment|currency_code|merchant_key|invoice_id
    const hashData = `${amount}|1|TRY|${merchantKey}|${invoiceId}`;
    const hashKey = generateHashKey(hashData, appSecret);
    ...
    const body = {
        ...
        response_method: 'GET',
        items: items || [{ name: 'Bakiye Yükleme', price: String(amount), quantity: 1, description: 'Cari hesap bakiyesi' }],
        name: name || 'B2B',
