...
export async function uploadToR2(buffer, fileName, contentType = 'image/webp', r2Config = null) {
    ...
    // Allow / so that folders work correctly
    ...
    let finalPublicUrl = `${publicUrl}/${cleanFileName}`;
    
    // Auto-fix if the publicUrl points to r2.dev ( Turkey ISP block )
    if (finalPublicUrl.includes('r2.dev')) {
        finalPublicUrl = finalPublicUrl.replace(/https:\/\/[^/]+\.r2\.dev/, 'https://cdn.omigroups.com');
    }
