...
export async function updateSession(request) {
    ...
    // 🚀 FAIL-SAFE: Trigger internal sync check via API (Edge-compatible)
    // Middleware içinde ağır mantık çalıştırmak yerine, arka planda API'yi dürtüyoruz.
    if (!request.nextUrl.pathname.startsWith('/_next') && !request.nextUrl.pathname.startsWith('/api')) {
        // "Fire and forget" - API kendi içinde tarih kontrolü yapıp gerekirse çalışacak.
        fetch(`${request.nextUrl.origin}/api/cron/sync-xml`, {
