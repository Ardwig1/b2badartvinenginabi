...
async function syncBasbugApi() {
    ...
    try {
        ...
        const loginRes = await axios.post('https://api.basbug.com.tr/auth/Login', authData);
        ...
        for (const brand of TARGET_BRANDS) {
            ...
            const [stockRes, priceRes] = await Promise.all([
                axios.get(`https://api.basbug.com.tr/material/StokGetir?ListeGrubu=${brand}&FirmaAdi=BASBUG&Depo=MRK`, { headers: { 'Authorization': `Bearer ${token}` } }),
                axios.get(`https://api.basbug.com.tr/material/FiyatGetir?ListeGrubu=${brand}&FirmaAdi=BASBUG`, { headers: { 'Authorization': `Bearer ${token}` } })
            ]);
