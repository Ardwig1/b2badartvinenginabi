...
/**
 * Basbug Group API Sync Engine (Clean & Reverted to Brand Logic)
 */
...
async function syncBasbugApi() {
    ...
    try {
        ...
        const apiConfig = { headers: { ...commonHeaders, 'Authorization': `Bearer ${token}` } };
        ...
        for (const brand of TARGET_BRANDS) {
            ...
            try {
                ...
                console.log(`✨ Found ${stockList.length} items.`);

                // Limit items per brand to prevent Vercel timeout (Initial test: 100)
                const itemsToProcess = stockList.slice(0, 100);
                ...
                for (const stockItem of itemsToProcess) {
                    ...
                    try {
                        ...
                        if (item && item.no) {
                            // Rich Name Format: [Brand] [Name] - [Models] ([Year])
                            ...
                            if (pricingRules[supplierBrand] !== undefined) activeMargin = Number(pricingRules[supplierBrand]);
                ...
                console.error(`❌ Failed ${brand}: ${err.message}`);
            }
        }

        if (allProductsToUpsert.length > 0) {
            console.log(`\n⬆️ Upserting ${allProductsToUpsert.length} items...`);
            const chunkSize = 100;
