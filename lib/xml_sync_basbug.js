...
async function syncBasbugApi() {
    ...
    console.log(`   → ${manualCodes.size} manuel ürün korunacak`);

    const newCodes = targetCodes.filter(c => !existingCodes.has(c));
    const updateCodes = targetCodes.filter(c => existingCodes.has(c) && !manualCodes.has(c));
    ...
    // Artık stokta olmayan ürünleri pasife al (manuel ürünlere dokunma)
    const targetSet = new Set(targetCodes);
    const toDeactivate = [...existingCodes].filter(c => !targetSet.has(c) && !manualCodes.has(c));
