    ...
    try {
        ...
        for (const brand of TARGET_BRANDS) {
            ...
            const [stockRes, priceRes] = await Promise.all([
                axios.get(`${PROXY_URL}/material/StokGetir?ListeGrubu=${encodeURIComponent(brand)}&FirmaAdi=BASBUG&Depo=MRK`, apiConfig),
                axios.get(`${PROXY_URL}/material/FiyatGetir?ListeGrubu=${encodeURIComponent(brand)}&FirmaAdi=BASBUG`, apiConfig)
            ]);
            ...
            // İlk kurulum için stok filtresini biraz gevşetelim veya en azından ilk 100 ürünü çekelim
            // Normalde: stockList.filter(s => (s.stok + s.sYol + s.sFarkliDepo) > 0);
            const activeItems = stockList.slice(0, 200); // Test amaçlı her markadan ilk 200 ürünü al
            console.log(`✨ Syncing first ${activeItems.length} items for ${brand} (including 0 stock for test).`);
            ...
            for (const stockItem of activeItems) {
                ...
                try {
                    // Detailed Info Request via Proxy
