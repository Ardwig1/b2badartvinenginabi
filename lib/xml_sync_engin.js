    ...
    try {
        ...
        for (let rawItem of rawItems) {
            ...
            if (item && item.STOK_KODU) {
                ...
                // Sistem kuralı: > 15 tam yeşil, <= 15 yarım yeşil.
                let stockQty = 0;
                if (stockStatus === 'VAR') {
                    stockQty = 100; // Tam Yeşil (>15)
                } else if (stockStatus === 'AZ VAR') {
                    stockQty = 15; // Yarım Yeşil (<=15)
                }
        ...
        // Manuel eklenen ürünleri (is_manual=true) güncelleme dışında tut
        const manualCodes = new Set();
        let manualFrom = 0;
        while (true) {
            const { data: mp } = await supabase.from('products').select('code').eq('is_manual', true).range(manualFrom, manualFrom + 999);
            if (!mp || mp.length === 0) break;
            mp.forEach(p => manualCodes.add(p.code));
            if (mp.length < 1000) break;
            manualFrom += 1000;
        }
        const filteredUpsert = productsToUpsert.filter(p => !manualCodes.has(p.code));
        console.log(`🔒 ${manualCodes.size} manuel ürün korundu, ${filteredUpsert.length} ürün güncelleniyor.`);

        // 4. Batch Upsert to Supabase
        const chunkSize = 500;
        console.log("⬆️ Syncing products to Supabase...");
        for (let i = 0; i < filteredUpsert.length; i += chunkSize) {
            const chunk = filteredUpsert.slice(i, i + chunkSize);
        ...
        while (true) {
            const { data: page, error: pageError } = await supabase
                .from('products')
                .select('code')
                .like('code', 'GKL%')  // sadece Gümüşkale ürünleri
