...
async function syncGumuskaleXml() {
    ...
    try {
        ...
        const pricingRules = marginSettings?.rules || {}; // { "GKL": 40, "OEM": 25 }
        ...
        for (let rawItem of rawItems) {
            ...
            if (item && item.STOK_KODU) {
                ...
                let productBrand = supplierBrand; // Varsayılan
                ...
                productsToUpsert.push({
                    ...
                    brand: productBrand, // Dönüştürülmüş marka (ARTEA, ARTEAN vb.)
                    car_brand: carBrand, // XML'deki Araç Markası (VOLKSWAGEN vb.)
                    supplier_brand: supplierBrand, // XML Stok Markası -> Gizli Tedarikçi
                    oem_no: String(item.OEM_KODU || '').trim(),
                    list_price: listPrice,
                    cost_price: costPrice,
                    profit_margin: activeMargin,
                    currency: String(item.PARA_BIRIMI || 'TRY').trim(),
                    stock_quantity: parseInt(item.STOK_ADETI) || 0,
                    stock_merkez: parseInt(item.STOK_ADETI) || 0,
                    is_active: true
                });
            }
        }
        ...
        const chunkSize = 500; // Smaller chunks for better stability
        ...
        const { data: dbProducts, error: fetchError } = await supabase.from('products').select('code');
        
        if (fetchError) {
            console.error("❌ Could not fetch DB products for cleanup:", fetchError.message);
        } else {
            const dbCodes = dbProducts.map(p => p.code);
            const codesToDelete = dbCodes.filter(c => !xmlProductCodes.includes(c));
            ...
            if (codesToDelete.length > 0) {
                for (let i = 0; i < codesToDelete.length; i += chunkSize) {
                    ...
                    const { error: delErr } = await supabase.from('products').delete().in('code', chunk);
